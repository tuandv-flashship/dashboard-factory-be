...
final class BunnyCDNAdapter implements FilesystemAdapter, PublicUrlGenerator, ChecksumProvider
{
    ...
    public function __construct(protected BunnyCDNClient $client, protected string $pullZoneURL = '')
    {
        if (func_num_args() > 2 && (string) func_get_arg(2) !== '') {
            throw new RuntimeException(
                'PrefixPath is no longer supported directly. Use PathPrefixedAdapter instead: ' .
                'https://flysystem.thephpleague.com/docs/adapter/path-prefixing/'
            );
        }
    }
    ...
    public function readStream(string $path)
    {
        ...
        } catch (BunnyCDNException|NotFoundException $exception) {
            throw UnableToReadFile::fromLocation($path, $exception->getMessage());
        }
    }
    ...
    public function fileSize(string $path): FileAttributes
    {
        ...
        if (! $filesize) {
            throw new UnableToRetrieveMetadata('Unknown file size');
        }
    ...
    public function lastModified(string $path): FileAttributes
    {
        ...
        if (! $timestamp) {
            throw new UnableToRetrieveMetadata('Unknown timestamp');
        }
    ...
    public function publicUrl(string $path, Config $config): string
    {
        if ($this->pullZoneURL === '') {
            throw new RuntimeException(
                'In order to get a visible URL for a BunnyCDN object, you must pass the "pullZoneURL" parameter.'
            );
        }
    ...
    {
        return (date_create_from_format('Y-m-d\TH:i:s.u', $timestamp) ?: date_create_from_format('Y-m-d\TH:i:s', $timestamp))
            ->getTimestamp();
    }
