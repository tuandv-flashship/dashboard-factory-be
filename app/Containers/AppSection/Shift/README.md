...
#### PATCH /v1/admin/shifts/{id}/hourly — Điều chỉnh nhân sự + máy in

Batch update hourly records. Hỗ trợ tất cả các fields: `kpi_minutes`, `target`, `staff_required`, `note`, `machine_count` (DTF), `active_machine_ids` (DTG).

**Request body (per_person):**
```json
{
  "records": [
    { "id": 101, "staff_required": 5 },
    { "id": 102, "note": "Giảm 1 người" }
  ]
}
```

**Request body (per_machine_dtf — thay đổi số máy):**
```json
{
  "records": [
    { "id": 201, "machine_count": 5 },
    { "id": 202, "machine_count": 3, "note": "Bảo trì 2 máy" }
  ]
}
```

**Request body (per_machine_dtg — chọn máy cụ thể):**
```json
{
  "records": [
    { "id": 301, "active_machine_ids": [1, 3] },
    { "id": 302, "active_machine_ids": [] }
  ]
}
```
...
| `records.*.target` | sometimes, nullable, integer, min:0 | Manual override |
| `records.*.staff_required` | sometimes, nullable, integer, min:0 | |
| `records.*.note` | sometimes, nullable, string, max:500 | |
| `records.*.machine_count` | sometimes, nullable, integer, min:0 | **Chỉ có tác dụng với per_machine_dtf và per_machine_dtg** |
| `records.*.active_machine_ids` | sometimes, array | **Chỉ có tác dụng với per_machine_dtg** |
| `records.*.active_machine_ids.*` | integer, exists:machines,id | Phải thuộc đúng department |

**Side effects:**

| Field thay đổi | Side effect |
|---|---|
| `kpi_minutes` | Auto-recalc `kpi_hours`, `kpi_percent` |
| `target` | Cascade `hour_start_inventory` cho các slot sau |
| `machine_count` (DTF) | Lưu trực tiếp, dùng làm multiplier fallback khi tính target |
| `active_machine_ids` (DTG) | Sync pivot `hourly_record_machines`, auto-set `machine_count` = count, auto-set `target` = Σ(KPIs) × kpi_percent (trừ khi gửi kèm `target` manual) |
...
```json
{
  "data": {
    ...
    "shift_detail": {
      ...
      "kpi_per_hour": 400,
      "machines": [
        { "id": "HASHED_M1", "name": "Apollo",   "code": "apollo",   "kpi_per_hour": 250 },
        { "id": "HASHED_M2", "name": "Atlas-01", "code": "atlas_01", "kpi_per_hour": 75 },
        { "id": "HASHED_M3", "name": "Atlas-02", "code": "atlas_02", "kpi_per_hour": 75 }
      ]
    }
  }
}
...
> 💡 **FE notes:**
> - `productivity_type` → quyết định render UI: `per_person` = input headcount, `per_machine_dtf` = input số máy, `per_machine_dtg` = checkbox chọn máy
> - `shift_detail.machines[]` = danh sách **tất cả máy available** của ca (từ `shift_detail_machines`). FE dùng làm nguồn cho checkbox
> - `machine_count` = số máy đang active cho khung giờ này (null = chưa override, dùng default từ ca)
> - **Available include:** `?include=hourlyMachines` → trả thêm `hourly_machines` array (danh sách máy được chọn override cho slot này)

**Include `hourlyMachines` response:**
```json
{
  "data": {
    "...": "...",
    "hourly_machines": {
      "data": [
        { "machine_id": "HASHED_M1", "code": "apollo",   "name": "Apollo",   "kpi_per_hour": 250 },
        { "machine_id": "HASHED_M2", "code": "atlas_01", "name": "Atlas-01", "kpi_per_hour": 75 }
      ]
    }
  }
}
...
#### PATCH /v1/admin/hourly-records/{id} — Sửa 1 khung giờ

Tương tự batch API nhưng cho 1 record. Request body trực tiếp (không cần wrap trong `records[]`).

**Request body (per_machine_dtf):**
```json
{
  "machine_count": 5,
  "note": "Tăng thêm 2 máy"
}
```

**Request body (per_machine_dtg):**
```json
{
  "active_machine_ids": [1, 3]
}
```

**Request body (manual target override):**
```json
{
  "active_machine_ids": [1, 3],
  "target": 999
...
| `target` | sometimes, nullable, integer, min:0 | Manual override — ưu tiên cao nhất |
| `staff_required` | sometimes, nullable, integer, min:0 | |
| `note` | sometimes, nullable, string, max:500 | |
| `machine_count` | sometimes, nullable, integer, min:0 | Chỉ DTF/DTG |
| `active_machine_ids` | sometimes, array | Chỉ DTG |
| `active_machine_ids.*` | integer, exists:machines,id | |

**Response:** `200 OK` — updated hourly record (same format as GET).
...
│   ├── UpdateHourlyStaffTask.php           ← +machine_count, +active_machine_ids pivot sync, cascade
...
└── UI/API/
    ...
    │   ├── FindHourlyRecordController.php       ← +eager-load hourlyMachines, +shift_detail context
    │   ├── GetShiftTemplateDefaultsController.php
    ...
    │   ├── UpdateSingleHourlyRecordController.php ← +eager-load hourlyMachines
    ...
    │   ├── UpdateHourlyStaffRequest.php          ← +machine_count, active_machine_ids rules
    │   ├── UpdateSingleHourlyRecordRequest.php   ← +machine_count, active_machine_ids rules
...
## FE Integration Guide — Sửa Giờ Làm Việc (Hourly Record Machine Override)

### Tổng quan

Khi mở modal sửa khung giờ, FE cần phân biệt **3 loại department** để hiển thị UI phù hợp:

| `productivity_type` | UI Component | Input field | Mô tả |
|---|---|---|---|
| `per_person` | Input số | `staff_required` | Nhập số nhân viên |
| `per_machine_dtf` | Input số | `machine_count` | Nhập tổng số máy DTF hoạt động |
| `per_machine_dtg` | Checkbox list | `active_machine_ids` | Chọn máy DTG cụ thể |

### Luồng hoạt động chi tiết

```
┌─────────────────────────────────────────────────────────────────┐
│ 1. User click vào 1 khung giờ → FE gọi:                        │
│    GET /v1/admin/hourly-records/{id}?include=hourlyMachines     │
│                                                                 │
│ 2. Response trả về:                                              │
│    ├── productivity_type: "per_machine_dtg"                     │
│    ├── machine_count: 2 (hoặc null = chưa override)            │
│    ├── shift_detail.machines[]: 3 máy available                 │
│    └── hourly_machines[]: 2 máy đang active cho slot này        │
│                                                                 │
│ 3. FE render form theo productivity_type:                       │
│    ├── per_person     → input staff_required                    │
│    ├── per_machine_dtf → input machine_count (number)           │
│    └── per_machine_dtg → checkbox từ shift_detail.machines[]    │
│         ├── ☑ Apollo (250/h) — checked vì có trong hourly_machines  │
│         ├── ☐ Atlas-01 (75/h) — unchecked                       │
│         └── ☑ Atlas-02 (75/h) — checked vì có trong hourly_machines │
│                                                                 │
│ 4. User thay đổi → FE gọi:                                      │
│    PATCH /v1/admin/hourly-records/{id}                          │
│    ├── DTF: { "machine_count": 5 }                              │
│    └── DTG: { "active_machine_ids": [1, 3] }                   │
│                                                                 │
│ 5. Server auto xử lý:                                            │
│    ├── DTF: lưu machine_count, dùng làm multiplier              │
│    └── DTG: sync pivot → machine_count = 2, target = 250+75     │
│                                                                 │
│ 6. Response trả về hourly record updated                         │
└─────────────────────────────────────────────────────────────────┘
```

### Logic xác định máy đang active cho 1 khung giờ

```
Máy active cho slot = hourly_record_machines (nếu có override)
                    ↓ fallback
                    = shift_detail_machines (mặc định từ ca)
```

| Trường hợp | `hourly_machines.data` | Nguồn |
|---|---|---|
| User đã override | `[Apollo, Atlas-02]` | Từ `hourly_record_machines` pivot |
| Chưa override | `[]` (empty array) | FE dùng `shift_detail.machines` làm mặc định |

> ⚠️ **FE cần check:** Nếu `hourly_machines.data` rỗng → hiển thị tất cả máy từ `shift_detail.machines[]` là "checked" (mặc định). Nếu có dữ liệu → chỉ checked những máy có trong `hourly_machines`.

### Hành vi `active_machine_ids` (DTG)

| Trường hợp | Request | Kết quả |
|---|---|---|
| Override 2 máy | `{"active_machine_ids": [1, 3]}` | `machine_count = 2`, `target = Σ(KPI máy 1 + máy 3) × kpi_percent` |
| Bỏ tất cả máy | `{"active_machine_ids": []}` | `machine_count = 0`, `target = 0` |
| Không gửi field | `{"note": "test"}` | Giữ nguyên máy cũ, không thay đổi |
| Kèm target manual | `{"active_machine_ids": [1], "target": 999}` | Sync máy nhưng target = 999 (manual ưu tiên) |

### Hành vi `machine_count` (DTF)

| Trường hợp | Request | Kết quả |
|---|---|---|
| Override 5 máy | `{"machine_count": 5}` | Lưu trực tiếp, dùng làm multiplier khi tính target |
| Reset về default | `{"machine_count": null}` | Xóa override, fallback về `shift_detail.machine_count` |
| Không gửi field | `{"note": "test"}` | Giữ nguyên giá trị cũ |
