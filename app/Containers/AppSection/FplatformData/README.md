...
Tất cả SQL tham chiếu nằm trong file `rpt_factory_ops_metrics_v8_1.sql` (2196 dòng, 24 nhóm query).
...
| ------------------- | ------------------- | ------------------------------------------------------------------- | ------- | --------------------------------- |
| Hotshot In          | `hotshot_print`     | `folder_manage` + `user_group_scan` (work_type=0)                   | file    | `printer_default = MayHOTSHOT/PD` |
| Hotshot Pick        | `hotshot_pick`      | `folder_manage` + `user_group_scan` (work_type=100, copy_job=0)     | product | `printer_default = MayHOTSHOT/PD` |
| Hotshot Cắt         | `hotshot_cut`       | `folder_manage` + `user_group_scan` (work_type=2)                   | file    | `printer_default = MayHOTSHOT/PD` |
| Hotshot Mockup      | `hotshot_mockup`    | `folder_manage` + `order_check_file_dropbox` + `log_check_mockup`   | file    | `printer_default = MayHOTSHOT/PD` |
| Hotshot Pack & Ship | `hotshot_pack_ship` | `folder_manage` + `order_check_file_dropbox` + `scan_label_history` | shirt   | `printer_default = MayHOTSHOT/PD` |
...
```
docs/
├── rpt_factory_ops_metrics_v8_1.sql   # [CHÍNH] 24 nhóm query, nguồn chính thức
